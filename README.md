...
* [`src/cljs/re_frame_tic_tac_toe/`](src/cljs/re_frame_tic_tac_toe/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/re_frame_tic_tac_toe/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/re_frame_tic_tac_toe/`](test/cljs/re_frame_tic_tac_toe/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `re-frame-tic-tac-toe` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/re_frame_tic_tac_toe/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns re-frame-tic-tac-toe.example
  (:require [re-frame-tic-tac-toe.config :as config])
